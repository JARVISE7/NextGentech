...
export default function ProductsPage() {
  ...
  const handleProductClick = (product) => {
    router.push(`/product/${product.id}`); // Redirect to product details page
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {products.map((product) => {
          ...
          return (
            <div
              key={product.id}
              className="border rounded-lg p-4 shadow-md bg-white relative group overflow-hidden cursor-pointer"
              onClick={() => handleProductClick(product)} // Redirect on product card click
            >
